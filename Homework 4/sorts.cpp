...
int main()
{
	...
	// destruction time.
	{
		...
		vector<Store*> unorderedPtrToStores;
		for (int i = 0; i < auxStores.size(); i++)
			unorderedPtrToStores.push_back(&auxStores[i]);

		// TODO:  Sort the vector of pointers using the STL sort algorithm
		//        with compareStorePtr as the ordering relationship.
		sort(unorderedPtrToStores.begin(),unorderedPtrToStores.end(), compareStorePtr);
		...
		for (int i = 0; i < unorderedPtrToStores.size(); i++)
			stores[i] = *unorderedPtrToStores[i];
